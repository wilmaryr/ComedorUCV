...
public class MenuDeLaSemana extends JFrame {
    ...
    public MenuDeLaSemana(Usuario u) {
        this.usuario = u;
        setTitle("Comedor UCV - Menú de la semana");
        setSize(1000, 700);
    ...
    private void initUI() {
        ...
        breakfastPanel.add(activeBar, BorderLayout.WEST);
        ...
        lblBreakfast.setFont(new Font("Segoe UI", Font.BOLD, 16));
        lblBreakfast.setForeground(PRIMARY_COLOR);
        lblBreakfast.setBorder(new EmptyBorder(0, 15, 0, 0));
        breakfastPanel.add(lblBreakfast, BorderLayout.CENTER);
        ...
        lblLunch.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        lblLunch.setForeground(Color.GRAY);
        lblLunch.setAlignmentX(Component.CENTER_ALIGNMENT);

        JPanel lunchPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 20, 0));
        lunchPanel.setOpaque(false);
        lunchPanel.setMaximumSize(new Dimension(220, 50));
        lunchPanel.add(lblLunch);
        lunchPanel.setCursor(new Cursor(Cursor.HAND_CURSOR));
        lunchPanel.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                new MenuDeLaSemanaTarde(usuario).setVisible(true);
                dispose();
            }
        });
        ...
        JLabel lblSectionTitle = new JLabel("Desayuno (7am/9am)");
        ...
        Map<String, Plato> menuDesayuno = loadMenuForTurno("DESAYUNO");
        String[] dias = { "LUNES", "MARTES", "MIERCOLES", "JUEVES", "VIERNES" };

        for (String d : dias) {
            Plato p = menuDesayuno.get(d);
            if (p != null) {
                ...
                double precio = GestorSaldosYMenu.consultarPrecio(d, "DESAYUNO");
                String precioFormateado = String.format(java.util.Locale.US, "%.2f bs", precio);

                daysListPanel.add(
                        createDayRow(d, desc + " | Precio: " + precioFormateado, estado, "Solicitar turno", btnCol));
            } else {
    ...
    private JPanel createDayRow(String day, String menuDesc, String status, String btnText, Color btnColor) {
        ...
        if (btnColor == BUTTON_BLUE) {
            actionBtn.addActionListener(e -> {
                Map<String, Plato> menu = loadMenuForTurno("DESAYUNO");
                Plato p = menu.get(day);
                if (p != null) {
                    DialogoSeleccionMenu dialog = new DialogoSeleccionMenu(this, usuario, p, day, "DESAYUNO");
