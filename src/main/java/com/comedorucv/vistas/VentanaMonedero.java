...
public class VentanaMonedero extends JFrame {
    ...
    public VentanaMonedero(Usuario u) {
        ...
        gbc.insets = new Insets(20, 8, 8, 8); // Añadir más espacio arriba
        RoundedButton btnSaldoPana = new RoundedButton("Saldo Pana (Apoyar amigo)", new Color(0, 82, 126), Color.WHITE, 18);
        btnSaldoPana.setPreferredSize(new Dimension(250, 40));
        contenido.add(btnSaldoPana, gbc);
        ...
        btnRecargar.addActionListener(e -> {
            ...
            if (ref != null && ref.trim().matches("\\d{6}")) {
                if (GestorSaldosYMenu.existeReferencia(ref.trim())) {
                    JOptionPane.showMessageDialog(this,
                            "ERROR: El número de referencia ya ha sido utilizado anteriormente.",
                            "Referencia Duplicada", JOptionPane.ERROR_MESSAGE);
                    return;
                }
                saldo += seleccionado.doubleValue();
                lblMonedero.setText(String.format(Locale.US, "Monedero: %.2f", saldo));
                GestorSaldosYMenu.guardarSaldo(usuarioActual.getCedula(), saldo,
                        "Recarga vía VentanaMonedero (Ref: " + ref.trim() + ")");
                GestorSaldosYMenu.registrarReferencia(ref.trim());
        ...
        btnSaldoPana.addActionListener(e -> {
            new DialogoSaldoPana(this, usuarioActual).setVisible(true);
            // Actualizar saldo visual por si hubo cambios
            saldo = GestorSaldosYMenu.leerSaldo(usuarioActual.getCedula());
            lblMonedero.setText(String.format(Locale.US, "Monedero: %.2f", saldo));
        });
